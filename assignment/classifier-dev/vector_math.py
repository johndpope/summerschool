...
def find_nn_cos(v, Wv, k=10):
    ...
    pass
    #### YOUR CODE HERE ####
    Z = np.linalg.norm(Wv, axis=1) * np.linalg.norm(v)  #--SOLUTION--
    ds = np.dot(Wv, v.T) / Z  #--SOLUTION--
    nns = np.argsort(-1*ds)[:k]  # sort descending, take best  #--SOLUTION--
    return nns, ds[nns]  # word indices, similarities  #--SOLUTION--
...
def analogy(vA, vB, vC, Wv, k=5):
    ...
    pass
    #### YOUR CODE HERE ####
    target_v = vB - vA + vC              #--SOLUTION--
    return find_nn_cos(target_v, Wv, k)  #--SOLUTION--
